...
/**
 * The controller that handles elements in the cameras view.
 * @class ElementKamerasController
 */
myAppController.controller('ElementCamerasController', function ($scope, $q, $routeParams, $location, $cookies, dataService) { 
    $scope.dataHolder.devices.filter = {deviceType: 'camera'};
    ...
    $scope.setFilter = function (filter) {
        ...
        if (!filter || empty) {// Remove filter
            angular.extend($scope.dataHolder.devices, {filter: {deviceType: 'camera'}});
            $cookies.filterElements = angular.toJson({});
        } else {// Set filter
            angular.extend(filter, {deviceType: 'camera'});
            angular.extend($scope.dataHolder.devices, {filter: filter});
            $cookies.filterElements = angular.toJson(filter);
        }
        $scope.allSettled();
     ...
     */
    $scope.searchMe = function () {
        $scope.autocomplete.results = dataService.autocomplete($scope.dataHolder.devices.collection, $scope.autocomplete);
        // Expand/Collapse the list
        if (!_.isEmpty($scope.autocomplete.results)) {
            $scope.expandAutocomplete('searchCameras');
        } else {
            $scope.expandAutocomplete();
        }
        // Reset filter q if is input empty
        ...
        console.log(" $scope.autocomplete.results",  $scope.autocomplete.results);
    };
});
